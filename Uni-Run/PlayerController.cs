...
public class PlayerController : MonoBehaviour {
   ...
   private void Update() {      // 사용자 입력을 감지하고 점프하는 처리
       ...
       // 마우스 왼쪽 버튼을 눌렀으며 && 최대 점프 횟수(2)에 도달하지 않았따면
       if (Input.GetMouseButtonDown(0) && jumpCount < 2) {
        ...
        // 마우스 왼쪽 버튼에서 손을 떼는 순간 && 속도의 y 값이 양수라면(위로 상승 중) 현재 속도를 절반으로 변경
   ...
   private void OnCollisionEnter2D(Collision2D collision) {       // 바닥에 닿았음을 감지하는 처리
        ...
        if (collision.contacts[0].normal.y > 0.7f) {    // 이 조건을 검사함으로써 '절벽'이나 '천장'을 '바닥'으로 인식하는 문제를 해결함
                                                        // 노말벡터의 방향을 검사
                                                        // collision.contacts[0]은 두 물체 사이의 여러 충돌 지점 중에서 첫 번째 충돌 지점의 정보를 가져옴
                                                        // ContactPoint와 ContactPoint2D 타입은 충돌 지점에서 충돌 표면의 방향(노말벡터)을 알려주는 변수인 normal을 제공한다.
            isGrounded = true;
            jumpCount = 0;      // isGrounded를 true로 변경하고, 누적 점프 횟수를 0으로 리셋
        }
   }
